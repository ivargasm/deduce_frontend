...
export async function register(username: string, email: string, password: string, accept_terms: boolean, url: string) {
    const res = await fetch(`${url}/auth/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, email, password, accept_terms }),
    });
